...
def run_plot():
    config_path = entry_config_path.get()
    if os.path.exists(config_path):
        config = read_config_file(config_path)
        data_path = config.get("输入文件路径")
        x = float(config.get("下分界线值", 1e-5))
        y = float(config.get("上分界线值", 2.806096e-8))
        filename = config.get("输出文件名", "out")
        out_path = config.get("输出文件路径", ".")
    else:
...
def browse_config_file():
    ...
    if config_path:
        entry_config_path.delete(0, tk.END)
        entry_config_path.insert(0, config_path)
...
# 创建输入框和标签
label_config_path = tk.Label(root, text="配置文件:")
label_config_path.grid(row=0, column=0, padx=10, pady=5)
entry_config_path = tk.Entry(root, width=75)
entry_config_path.insert(0, "/path/to/your/config.txt")
entry_config_path.grid(row=0, column=1, padx=10, pady=5)

button_browse_config = tk.Button(root, text="浏览", command=browse_config_file)
button_browse_config.grid(row=0, column=2, padx=10, pady=5)

label_data_path = tk.Label(root, text="输入文件:")
label_data_path.grid(row=1, column=0, padx=10, pady=5)
entry_data_path = tk.Entry(root, width=75)
entry_data_path.insert(0, "/path/to/your/data.csv")
entry_data_path.grid(row=1, column=1, padx=10, pady=5)
...
label_y.grid(row=3, column=0, padx=10, pady=5)
...
label_filename.grid(row=4, column=0, padx=10, pady=5)
...
label_out_path.grid(row=5, column=0, padx=10, pady=5)
entry_out_path = tk.Entry(root, width=75)
entry_out_path.insert(0, "")
entry_out_path.grid(row=5, column=1, padx=10, pady=5)

# 创建按钮
button_run = tk.Button(root, text="绘图", command=run_plot)
button_exit = tk.Button(root, text="退出", command=exit_program)

button_run.grid(row=6, column=1, sticky="w", padx=10, pady=20)
button_exit.grid(row=6, column=1, sticky="e", padx=10, pady=20)
